...
function drawCanvas(context, r, checks) {
    ...
    context.fillStyle = "#453edf";
    context.fillRect(width / 5 * 2.5, height/10, width / 5*2, height / 5 * 2);//квадрат
    context.beginPath();
    context.moveTo(width / 2, height / 2);
    context.lineTo(width / 10, height / 5 * 2.5);
    context.lineTo(width / 2, height / 5 * 1.5);//треугл
    context.fill();
    context.moveTo(width /2, height / 2);
    context.arc(width / 2, width / 2, width / 2.5, Math.PI/2, Math.PI); //круг
    context.fill();
    context.beginPath();
